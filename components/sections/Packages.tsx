    ...
    <Section id="pakete" tone="white" ariaLabel={t('packagesTitle')}>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-3">
        {packageKeys.map((key, index) => {
          ...
          return (
            <article key={key} className="flex flex-col rounded-xl bg-cream p-6 ring-1 ring-navy/10 sm:p-8">
              ...
              <dl className="mt-5 space-y-3 border-b border-navy/10 pb-5">
                <div>
                  <dt className="text-xs uppercase tracking-wider text-navy/60">{t('setupFee')}</dt>
                  <dd className="text-2xl font-semibold text-navy">{t(`packages.${key}.setup`)}</dd>
                </div>
                <div>
                  <dt className="text-xs uppercase tracking-wider text-navy/60">{t('monthlyFee')}</dt>
                  <dd className="text-lg font-semibold text-navy">{t(`packages.${key}.monthly`)}</dd>
                </div>
              </dl>

              <p className="mt-5 text-xs font-medium uppercase tracking-wider text-navy/60">
                {previous ? t('includesPrevious', { name: previous }) : t('includes')}
              </p>
              <ul className="mt-3 space-y-2">
                {features.map((feature) => (
                  <li key={feature} className="flex gap-2.5 text-sm leading-relaxed text-navy/80">
      ...
      <div className="mt-10 max-w-3xl rounded-xl border border-navy/10 p-6">
